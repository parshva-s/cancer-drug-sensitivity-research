...
def grid_search_random_forest(dataset_name, drug_id, method, k, X_train, y_train, rf_param_grid=None):
    """
    Perform GridSearchCV for Random Forest Regressor.
    If no rf_param_grid is provided, default grid parameters will be used.
    """
    rf_model_path = f'{models_directory}{dataset_name}/{drug_id}_{method}_{k}_rf.pkl'
    grid_search_rf = None
...
def grid_search_svr(dataset_name, drug_id, method, k, X_train, y_train, svr_param_grid=None):
    """
    ...
    """
    svr_model_path = f'{models_directory}{dataset_name}/{drug_id}_{method}_{k}_svr.pkl'
...
def grid_search_elastic_net(dataset_name, drug_id, method, k, X_train, y_train, en_param_grid=None):
    """
    Perform GridSearchCV for Elastic Net Regressor.
    If no en_param_grid is provided, default grid parameters will be used.
    """
    en_model_path = f'{models_directory}{dataset_name}/{drug_id}_{method}_{k}_en.pkl'
    grid_search_en = None
...
def train_neural_network(dataset_name, drug_id, method, k, X_train, y_train, X_val, y_val, epochs=100, batch_size=32):
    """
    ...
    nn_model_path = f'{models_directory}{dataset_name}/{drug_id}_{method}_{k}_nn.h5'
...
def train_gnn_model(dataset_name, drug_id, method, k, X_train, y_train, X_val, y_val, hidden_dim=64, epochs=25):
    """
    ...
    model_path = f"{models_directory}{dataset_name}/{drug_id}_{method}_{k}_gnn.pth"
